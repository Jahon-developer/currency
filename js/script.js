    ...
    const flagUrls = {
        ...
        "EUR": "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b7/Flag_of_Europe.svg/1200px-Flag_of_Europe.svg.png", // Yevro
    ...
    await data.map((card) => {
        const cardElement = `<div class="all_currency_item">
                ...
                <div class="all_currency_item_text">
                    ...
                    <h4>${card.Diff} so'm</h4>
                </div>
                <div class="all_currency_item_icon">
                        <img src="${card.Diff > 0 ? `https://cbu.uz/bitrix/templates/main/img/svg/icon__chart_up.svg` : card.Diff < 0 ? `https://cbu.uz/bitrix/templates/main/img/svg/icon__chart_down.svg` : card.Diff == 0 ? `https://cbu.uz/bitrix/templates/main/img/svg/icon__chart_zero.svg` : "" }" 
                        alt="">
                </div>
            </div>`
...
document.querySelector("#FromSelect").addEventListener("change", async (e) => {
    const data = await fetch("https://cbu.uz/uz/arkhiv-kursov-valyut/json/").then((res) => res.json()).then((data) => {
        return data
    })
    let FromInput = Number(e.target.value)
...
document.querySelector("#toSelect").addEventListener("change", async (e) => {
    const data = await fetch("https://cbu.uz/uz/arkhiv-kursov-valyut/json/").then((res) => res.json()).then((data) => {
        return data
    })
    let FromInput = Number(e.target.value)
